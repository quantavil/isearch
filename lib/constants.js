...
const BLOCKED_URLS = [ // URL patterns to block via Chrome DevTools Protocol for faster page loads
    ...
    '*.css', // Stylesheets (major performance win for text-only extraction)
    '*.woff', '*.woff2', '*.ttf', '*.otf', // Font files
    '*.mp4', '*.webm', '*.mp3', // Media files
    'manifest.json', 'opensearch.xml', // Metadata
    ...
    '*consent*', '*cookie*', '*quantserve*', '*scorecardresearch*', '*zedo*', // Consent dialogs, trackers
    '*adservice*', '*client_204*', '*gen_204*' // Google specific tracking
];
